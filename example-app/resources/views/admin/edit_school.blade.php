        ...
        <div class="flex flex-col border-4 border-black shadow-2xl bg-white">
            ...
            <div class="flex flex-col border-b-4 border-black">
                ...
                <div class="p-8 space-y-6 border-b-2 border-slate-100 bg-white">
                    <h3 class="text-xs font-black text-slate-500 uppercase mb-2">Utility Connectivity</h3>
                    
                    <div class="border-2 border-slate-200 p-3 bg-white">
                        <span class="text-sm font-bold text-slate-600 uppercase block mb-2">Power Supply Type</span>
                        <select name="with_electricity" class="w-full bg-[#a52a2a] text-white text-sm font-black uppercase px-3 py-3 outline-none cursor-pointer hover:bg-black transition-colors">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4">
                        <div class="flex items-center justify-between border-2 border-slate-200 p-3 bg-white">
                            <span class="text-sm font-bold text-slate-600 uppercase">Potable Water</span>
                            <select name="with_potable_water" class="bg-[#a52a2a] text-white text-sm font-black uppercase px-3 py-2 outline-none cursor-pointer hover:bg-black transition-colors w-32 text-center">
                                <option value="1" {{ $school->with_potable_water ? 'selected' : '' }}>Yes</option>
                                <option value="0" {{ !$school->with_potable_water ? 'selected' : '' }}>No</option>
                            </select>
                        </div>
                        <div class="flex items-center justify-between border-2 border-slate-200 p-3 bg-white">
                            <span class="text-sm font-bold text-slate-600 uppercase">Internet Connectivity</span>
                            <select name="with_internet" class="bg-[#a52a2a] text-white text-sm font-black uppercase px-3 py-2 outline-none cursor-pointer hover:bg-black transition-colors w-32 text-center">
                ...
                <div class="p-8 bg-slate-50/30 flex-grow relative">
                    <h3 class="text-xs font-black text-slate-500 uppercase mb-4 tracking-widest">Calculated Deficits</h3>
                    <div class="space-y-6 max-w-lg">
                        @foreach([['name' => 'classroom_shortage', 'label' => 'Classroom Shortage'], ['name' => 'chair_shortage', 'label' => 'Chair Shortage'], ['name' => 'toilet_shortage', 'label' => 'Sanitation Shortage']] as $short)
                        <div class="flex flex-col border-b-2 border-slate-200 pb-3 relative">
                            <div class="flex items-center justify-between z-10 relative">
                                <span class="text-sm font-bold text-slate-600 uppercase tracking-tight">{{ $short['label'] }}</span>
                                <input type="number" name="{{ $short['name'] }}" id="input_{{ $short['name'] }}" value="{{ $school->{$short['name']} ?? 0 }}" 
                                       class="w-32 bg-white border-2 border-slate-200 font-mono text-base font-black p-2 text-right focus:border-[#a52a2a] outline-none text-[#a52a2a] transition-all hover:border-[#a52a2a]/50">
                            </div>
                            <div class="flex justify-end mt-1.5 z-0 relative">
                                <span id="suggestion_{{ $short['name'] }}" class="text-xs text-slate-500 font-bold uppercase tracking-widest text-right flex items-center gap-1 transition-colors">
                ...
                <div class="p-8 bg-slate-50/30 flex-grow space-y-4">
                    <div>
                        ...
                        <select id="hazard_selector" name="hazard_type" onchange="toggleHazardInput(this.value)" 
                                class="w-full bg-[#a52a2a] text-white border-4 border-[#a52a2a] p-4 text-sm font-bold uppercase focus:outline-none hover:bg-black transition-colors cursor-pointer">
                    ...
                    <div id="other_hazard_container" class="{{ in_array($school->hazard_type, ['None', 'Flood Prone', 'Landslide Risk', 'Seismic Zone']) ? 'hidden' : '' }}">
                        <label class="block text-xs font-black text-slate-500 uppercase mb-2 tracking-widest mt-6">Technical Remarks</label>
                        <textarea name="hazards" id="hazard_textarea" rows="4" 
                                  class="w-full p-4 border-2 border-slate-200 font-mono text-sm font-bold uppercase focus:outline-none focus:border-4 focus:border-[#a52a2a] focus:bg-white transition-all">{{ $school->hazards }}</textarea>
                    </div>
                </div>
            </div>
        </div>
    ...
    function calculateGuidedSuggestions() {
        ...
        const updateSuggestionUI = (type, calculatedValue, ratioText) => {
            ...
            let statusBadge = overrides[type] 
                ? `<span class="bg-[#a52a2a] text-white px-1.5 py-0.5 rounded text-[10px] ml-2 uppercase">Manual Override</span>` 
                : `<span class="bg-emerald-500 text-white px-1.5 py-0.5 rounded text-[10px] ml-2 uppercase">Auto-Synced</span>`;
