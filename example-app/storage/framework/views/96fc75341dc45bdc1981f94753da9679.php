...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="print-only official-print-header">
        <div class="flex justify-between items-center mb-6">
            ...
            <div class="text-right flex flex-col items-end">
                ...
                <p class="text-[10px] font-mono font-bold text-slate-600 uppercase leading-none">Serial: REG-<?php echo e(now()->format('Ymd')); ?>-<?php echo e(strtoupper(Str::random(4))); ?></p>
            </div>
        </div>
        ...
        <div class="flex justify-between items-end mt-8">
            ...
            <div class="text-right">
                ...
                <p class="text-sm font-bold text-slate-900"><?php echo e(now()->format('F d, Y h:i A')); ?></p>
            </div>
        </div>
    </div>
    ...
    <div class="no-print flex flex-col xl:flex-row justify-between items-start xl:items-center mb-10 gap-6">
        <div class="w-full xl:w-auto flex-1">
            ...
            <form action="<?php echo e(route('admin.schools')); ?>" method="GET" class="flex flex-wrap gap-3 w-full max-w-5xl search-form items-center">
                ...
                <div class="relative flex-1 min-w-[180px] flex items-center bg-white border-2 border-slate-200 rounded-2xl focus-within:border-[#a52a2a] transition-all shadow-sm pl-4 pr-3 py-1">
                    ...
                    <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search ID or Name..." 
                ...
                <div class="flex items-center bg-white border-2 border-slate-200 rounded-2xl shadow-sm focus-within:border-[#a52a2a] transition-all px-3">
                    <select name="sector" onchange="this.form.submit()" class="py-3 pr-2 bg-transparent font-bold text-[10px] uppercase tracking-widest text-slate-700 outline-none cursor-pointer">
                        ...
                        <option value="Public" <?php echo e(request('sector') == 'Public' ? 'selected' : ''); ?>>Public Schools</option>
                        <option value="Private" <?php echo e(request('sector') == 'Private' ? 'selected' : ''); ?>>Private Schools</option>
                    </select>
                </div>
                ...
                <div class="flex items-center bg-white border-2 border-slate-200 rounded-2xl shadow-sm focus-within:border-[#a52a2a] transition-all px-3">
                    <select name="level" onchange="this.form.submit()" class="py-3 pr-2 bg-transparent font-bold text-[10px] uppercase tracking-widest text-slate-700 outline-none cursor-pointer">
                        ...
                        <option value="Primary" <?php echo e(request('level') == 'Primary' ? 'selected' : ''); ?>>Primary (Elem)</option>
                        <option value="Secondary" <?php echo e(request('level') == 'Secondary' ? 'selected' : ''); ?>>Secondary (HS)</option>
                    </select>
                </div>
                    ...
                    <select name="district" onchange="this.form.submit()" class="py-3 pr-2 bg-transparent font-bold text-[10px] uppercase tracking-widest text-slate-700 outline-none cursor-pointer">
                        ...
                        <?php if(isset($districts)): ?>
                            <?php $__currentLoopData = $districts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $dist): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <option value="<?php echo e($dist); ?>" <?php echo e(request('district') == $dist ? 'selected' : ''); ?>><?php echo e($dist); ?></option>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                        <?php endif; ?>
                    </select>
                </div>

                <?php if(request('search') || request('sector') || request('level') || request('district')): ?>
                    <a href="<?php echo e(route('admin.schools')); ?>" class="flex items-center text-red-500 hover:text-red-700 transition-colors px-2" title="Clear Filters">
        ...
        <div class="flex flex-wrap gap-4 items-center shrink-0 mt-4 xl:mt-0">
            ...
            <a href="<?php echo e(route('schools.archive')); ?>" class="bg-white border-2 border-slate-200 text-slate-600 px-5 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:border-amber-500 hover:text-amber-600 transition-all flex items-center gap-2 shadow-sm">
            ...
            <a href="<?php echo e(route('schools.create')); ?>" class="bg-[#a52a2a] text-white px-6 py-3 rounded-2xl font-black uppercase text-[10px] tracking-widest shadow-lg shadow-red-900/20 hover:bg-black transition-all whitespace-nowrap">
                Add New
            </a>
        </div>
    </div>
                ...
                <?php $__empty_1 = true; $__currentLoopData = $schools; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $school): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <tr class="border-b border-slate-50 hover:bg-slate-50/50 transition-colors print:border-slate-300 print-break-inside-avoid">
                        ...
                        <td class="p-6 border-r border-slate-50 print:border-slate-300 print:py-3">
                            <span class="font-black text-slate-800 uppercase tracking-tight block"><?php echo e($school->name); ?></span>
                        </td>
                        <td class="p-6 border-r border-slate-50 print:border-slate-300 print:py-3">
                            <div class="flex flex-wrap gap-1 mb-1">
                                
                                <?php if(isset($school->sector) && $school->sector === 'Private'): ?>
                                ...
                                <span class="inline-block px-2 py-0.5 bg-slate-100 text-slate-700 border border-slate-200 text-[8px] font-black uppercase tracking-widest rounded-md print:border-slate-400">
                                    <?php echo e($school->school_level ?? 'N/A'); ?>

                                </span>
                            </div>
                            <span class="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mt-1 print:text-slate-800"><i class="bi bi-geo-alt-fill"></i> <?php echo e($school->district ?? 'No District'); ?></span>
                        </td>
                        <td class="p-6 border-r border-slate-50 print:border-slate-300 text-center font-bold tabular-nums print:text-slate-900 print:py-3">
                            <?php echo e(number_format($school->no_of_teachers)); ?>

                        </td>
                        <td class="p-6 border-r border-slate-50 print:border-slate-300 text-center font-bold tabular-nums print:text-slate-900 print:py-3">
                            <?php echo e(number_format($school->no_of_enrollees)); ?>

                        </td>
                        <td class="p-6 text-center no-print">
                            <div class="flex justify-center items-center gap-4">
                                <a href="<?php echo e(route('schools.edit', $school->id)); ?>" 
                                ...
                                <form action="<?php echo e(route('schools.destroy', $school->id)); ?>" method="POST" onsubmit="return confirm('Archive this institution?')">
    ...
    <div class="print-only mt-16 print-break-inside-avoid" id="printSignatureSection">
        ...
        <div class="mt-16 flex flex-col items-center gap-2 border-t border-slate-300 pt-6">
            ...
            <p class="text-[8px] text-slate-500 italic font-mono">System-generated verifiable document. E-Code: <?php echo e(strtoupper(Str::random(16))); ?></p>
        </div>
    </div>
...
<div id="importModal" class="fixed inset-0 z-[2000] hidden flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 transition-all opacity-0 pointer-events-none no-print" style="transition: opacity 0.3s ease;">
    ...
    <div class="bg-white rounded-[2rem] border border-slate-200 shadow-2xl w-full max-w-lg overflow-hidden flex flex-col transform scale-95 transition-transform duration-300" id="importModalContent">
        ...
        <form action="<?php echo e(route('schools.import')); ?>" method="POST" enctype="multipart/form-data" class="flex flex-col" id="importForm">
            <?php echo csrf_field(); ?>
            <div class="p-8">
                <div class="mb-8 flex gap-4 text-left bg-blue-50/50 p-5 rounded-2xl border border-blue-100">
                    ...
                    <div>
                        ...
                        <ol class="text-xs text-blue-900/80 font-medium space-y-1.5 list-decimal list-inside marker:text-blue-400 marker:font-black">
                            <li><a href="<?php echo e(route('schools.sample')); ?>" class="underline decoration-blue-300 hover:text-[#a52a2a] transition-colors font-bold">Download the CSV template</a>.</li>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\deped\example-app\resources\views/admin/schools.blade.php ENDPATH**/ ?>